...
public class StudentDAO {
    ...
    public boolean updateGroupFieldId(int groupId, int newFieldId) {
        try {
            // First verify the field exists in faculty table (or wherever fields are stored)
            String checkQuery = "SELECT 1 FROM faculty WHERE field_id = ?";
            PreparedStatement checkPs = conn.c.prepareStatement(checkQuery);
            checkPs.setInt(1, newFieldId);
            ResultSet rs = checkPs.executeQuery();
            
            if (!rs.next()) {
                JOptionPane.showMessageDialog(null, "Invalid Field ID: " + newFieldId);
                return false;
            }

            String updateQuery = "UPDATE group_details SET field_id = ? WHERE group_id = ?";
    ...
    // Add this method to StudentDAO.java
public boolean approveGroup(int groupId) {
    try {
        String query = "UPDATE group_details SET is_approved = 1 WHERE group_id = ?";
        PreparedStatement ps = conn.c.prepareStatement(query);
        ps.setInt(1, groupId);
...
// Add this method to check if mentor can approve the group
public boolean canMentorApproveGroup(int mentorId, int groupId) {
    try {
        String query = "SELECT 1 FROM group_details WHERE group_id = ? AND mentor_id = ? AND is_approved = 0";
        PreparedStatement ps = conn.c.prepareStatement(query);
        ps.setInt(1, groupId);
    ...
    public boolean deleteGroupById(int groupId) {
        ...
        try {
            ...
            // Step 1: Set group_id to NULL in Student table
            PreparedStatement updateStudentPs = conn.c.prepareStatement(updateStudentQuery);
            updateStudentPs.setInt(1, groupId);
            updateStudentPs.executeUpdate();
